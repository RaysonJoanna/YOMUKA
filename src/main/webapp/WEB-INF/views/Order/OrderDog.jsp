	...
	<main>
		<div class="common container" style="text-align: left">
			<button class="btn btn-lg button" style="margin-top: 5px;"
				onclick="animal('dog')">강아지</button>
			<a href="/OrderCat"><button class="btn btn-lg button"
					style="margin-top: 5px;">고양이</button></a> <a href="/OrderSmall">
				<button class="btn btn-lg button" style="margin-top: 5px;">소동물</button>
			</a> <br>
			<%-- 강아지 --%>
			<form id="productForm" action="/OrderDog/product" method="get">
				<input type="hidden" id="productTypeInput" name="productType">
			</form>
			<button class="btn btn-lg button dog button-button"
				onclick="setProductType('건식사료')">건식사료</button>
			<button class="btn btn-lg button dog button-button"
				onclick="setProductType('습식사료')">습식사료</button>
			<button class="btn btn-lg button dog button-button"
				onclick="setProductType('간식')">간식</button>
			<button class="btn btn-lg button dog button-button"
				onclick="setProductType('장난감')">장난감</button>
			<button class="btn btn-lg button dog button-button"
				onclick="setProductType('배변용품')">배변용품</button>
		</div>
		<div class="album container-fluid">
			<div class="common">
				<div class="row">
					<%-- JSTL로 데이터 출력 --%>
					<c:forEach items="${dogProduct}" var="product">
						<div class="col card-margin center">
							<form method="post" action="/OrderGudok/set">
								<input type="hidden" name="productQuantity" id="goodsRepImgUrl"	value="1"> 
								...
								<div class="card card-default">
									<a class="a-order" href="/OrderDetail?productNum=${product.productNum}&productImg=${product.productImg}&productName=${product.productName}&productPrice=${product.productPrice}">
