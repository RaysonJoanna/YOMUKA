...
<body>
	...
	<main>
		<div class="common container" style="text-align: left">
			<a href="/OrderDog"><button class="btn btn-lg button" style="margin-top: 5px;">강아지</button></a>
			<button class="btn btn-lg button" style="margin-top: 5px;" onclick="animal('cat')">고양이</button>
			<a href="/OrderSmall"><button class="btn btn-lg button" style="margin-top: 5px;">소동물</button> </a> <br>
			<%-- 고양이 --%>
			<form id="productForm" action="/OrderCat/product" method="get">
				<input type="hidden" id="productTypeInput" name="productType">
			</form>
			<button class="btn btn-lg button cat button-button"
			...
			<button class="btn btn-lg button cat button-button"
				onclick="setProductType('스크래쳐')">스크래쳐</button>
			<button class="btn btn-lg button cat button-button"
				onclick="setProductType('모래')">모래</button>
		</div>
		<div class="album container-fluid">
			<div class="common">
				<div class="row">
					<%-- JSTL로 데이터 출력 --%>
					<c:forEach items="${catProduct}" var="product">
